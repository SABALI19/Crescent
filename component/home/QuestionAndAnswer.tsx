...
export default function FAQPage() {
  ...
  const questionsData: Record<Category, QuestionItem[]> = {    "General Question": [
      {
  ...
  const Question = ({ question, answer }: QuestionItem) => {
          ...
          <div className="flex items-center">
            <motion.div
              className={`w-10 h-10 flex items-center justify-center rounded-full mr-4 transition-all ${
                isOpen ? "bg-green-500" : "bg-gray-700"
              }`}
  ...
  return (
    <div className="bg-gradient-to-r from-gray-900 via-green-900 to-gray-800 min-h-screen py-16 px-4 sm:px-6 relative z-50 overflow-hidden">
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-0 left-0 w-64 h-64 bg-green-500/10 rounded-full filter blur-3xl opacity-20 animate-float"></div>
        <div className="absolute bottom-0 right-0 w-96 h-96 bg-gray-700/10 rounded-full filter blur-3xl opacity-20 animate-float-delayed"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="inline-block relative mb-5">
            <span className="text-green-400 text-sm font-semibold tracking-wider uppercase bg-green-400/10 px-4 py-1.5 rounded-full inline-flex items-center">
              <FiPlay className="text-green-400 mr-2 text-sm" />
              FAQ
            </span>
            <div className="absolute inset-0 bg-green-400 rounded-full blur-lg opacity-20 -z-10 animate-pulse"></div>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
                ...
                {categories.map((item, index) => (
                  <li key={index}>
                    <motion.button
                      className={`flex items-center w-full p-4 rounded-lg text-lg transition-all duration-300 ${
                        selectedCategory === item
                          ? "text-white bg-gradient-to-r from-green-500/20 to-gray-700/20 shadow-lg shadow-green-500/10"
                    ...
                    >
                      ...
                      {selectedCategory === item && (
                        <motion.span 
                          className="ml-auto h-2 w-2 bg-green-400 rounded-full"
          ...
          >
            <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl shadow-2xl shadow-gray-900/50 overflow-hidden h-full">
              <div className="p-1">
                <div className="bg-gradient-to-r from-green-500/10 to-gray-700/10 p-6">
